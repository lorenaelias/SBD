-- lORENA DA SILVA ELIAS
-- VICTOR HUGO EUSTAQUIO LOPES
-- YAGO VINICIUS FERREIRA DE CASTRO
...
SELECT *
FROM CLIENTE
-- 1	"João José da Silva"	"Rua das Dores, 1687, Ipiranga"	"(91) 3524-4651"	"(91) 9254-4651"
-- 2	"Maria Joaquina Magalhães"	"Av. Litorânia, 5484, Praia do Morro"	"(28) 3842-9741"	"(28) 9954-9741"
-- 3	"Abelita Valéria Fernandes"	"Av. Marechal Deodoro da Fonseca, 842, Santa Maria"	"(21) 2164-8974"	"(21) 9988-8974"
-- 4	"Belarmino Jhones"	"Rua Joaquim Aníbal, 21, Centro"	"(31) 3249-1245"	"(31) 8821-1245"
-- 5	"Bosco Calcido das Cruzes"	"Rua da Curva, 10, Pelorinho"	"(71) 3984-5566"	"(71) 9287-5566"
-- 6	"Delfim Vieira Neto"	"Av. Dr. Roberto Luzitano, 5543, Ala Oeste"	"(55) 3218-7138"	"(55) 8671-7138"
-- 7	"Gabriela Bochelli Soviersovisk"	"Av. Atlântica, 1111, Orla"	"(98) 3285-4712"	"(98) 8137-4712"

-- 2)
SELECT 
	DISTINCT A.NOMEREAL, A.DATANASC
FROM 
	ATOR A, ESTRELA E
WHERE 
	E.CODATOR = A.COD
...
FROM
	FILME
WHERE
	CATEGORIA LIKE '%Romance%'
-- "Les Misérables"

-- 6)
SELECT
	A.NOMEARTISTICO, NOMEREAL
FROM 
	ATOR A, FILME, ESTRELA E
WHERE
	FILME.NUMFILME = E.NUMFILME AND
	E.CODATOR = A.COD AND
	FILME.TITULO_ORIGINAL LIKE 'The Godfather'
-- "Marlon Brando"	"Marlon Brando Jr."
-- "Al Pacino"	"Alfredo James Pacino"
-- "James Caan"	"James Edmund Caan"

-- 7)
SELECT
	TITULO_ORIGINAL, CATEGORIA
FROM
	FILME F, ATOR A, ESTRELA E
WHERE
	E.CODATOR = A.COD AND
	F.NUMFILME = E.NUMFILME AND
	A.NOMEREAL LIKE 'John Joseph Nicholson'

-- 8)
SELECT
FILME.TITULO_ORIGINAL, FILME.CATEGORIA
FROM
	FILME, CLIENTE, EMPRESTIMO
WHERE 
	FILME.NUMFILME = EMPRESTIMO.NUMFILME AND
	EMPRESTIMO.CLIENTE = CLIENTE.NUMCLIENTE AND
...
WHERE
	FILME.NUMFILME = EMPRESTIMO.NUMFILME AND
	EMPRESTIMO.DATARET > '2013/02/01' AND
	EMPRESTIMO.DATARET < '2013/02/28'
-- "The Avengers"
-- "Citizen Kane"
-- "Django Unchained"
...
SELECT 
	CLIENTE.NOME
FROM 
	CLIENTE, EMPRESTIMO, FILME
WHERE
	EMPRESTIMO.NUMFILME = FILME.NUMFILME AND
	EMPRESTIMO.CLIENTE = CLIENTE.NUMCLIENTE AND
	FILME.TITULO_ORIGINAL LIKE 'The Avengers'
-- "Abelita Valéria Fernandes"
-- "Belarmino Jhones"
...
SELECT
	ATOR.NOMEREAL
FROM
	ATOR,FILME,EMPRESTIMO, ESTRELA, CLIENTE
WHERE
	ATOR.COD = ESTRELA.CODATOR AND
	ESTRELA.NUMFILME = FILME.NUMFILME AND
	FILME.NUMFILME = EMPRESTIMO.NUMFILME AND
	EMPRESTIMO.CLIENTE = CLIENTE.NUMCLIENTE AND
	CLIENTE.NOME LIKE 'Abelita Valéria Fernandes'
ORDER BY
	ATOR.NOMEREAL ASC;
-- "Amanda Michelle Seyfried"
-- "Christopher Allen Lloyd"
-- "Daniel Michae DeVito Jr."
...
--13)
SELECT 
	FILME.DATA_LANCAMENTO, FILME.TITULO_ORIGINAL
FROM
	FILME, CLASSIFICACAO
WHERE 
   FILME.CLASSIFICACAO = CLASSIFICACAO.COD AND
   CLASSIFICACAO.NOME = 'Super-lançamento'
   
-- "2013-05-24"	"The Hangover Part III"
-- "1991-05-31"	"The Silence of the Lambs"
-- "2013-01-11"	"Les Misérables"
-- "2013-06-21"	"World War Z"
-- "2013-01-18"	"Django Unchained"
...
CREATE TABLE ATORESCONSAGRADOS(
	...
	datanasc DATE,
	CONSTRAINT PK_ATORESCONSAGRADOS PRIMARY KEY (codigo)
);

INSERT INTO AtoresConsagrados
SELECT COD,NOMEREAL,NOMEARTISTICO,NACIONALIDADE,DATANASC
FROM ATOR
WHERE ATOR.DATANASC < '1974/01/01'